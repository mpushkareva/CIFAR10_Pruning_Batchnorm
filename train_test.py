...
def train_model(network, network_name, train_loader, test_loader, n_epochs, learning_rate, device):
    ...
    test(network, test_loader, loss_function=nn.NLLLoss())
    for epoch in tqdm_notebook(range(n_epochs)):
        train_loss, train_correct, train_eval_loss, train_eval_correct = train_one_epoch(network, train_loader, epoch, optimizer, loss_function=nn.NLLLoss())
        test_loss, test_correct = test(network, test_loader, loss_function=nn.NLLLoss())
